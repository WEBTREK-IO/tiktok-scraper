...
import { constant as CONST } from './constant';
...
const promiseScraper = async (
  input: string,
  type: ScrapeType,
  options = {} as Options,
): Promise<Result | void | boolean> => {
...
export const hashtag = async (input: string, options?: Options): Promise<Result | void | boolean> =>
  promiseScraper(input, 'hashtag', options);
export const user = async (input: string, options?: Options): Promise<Result | void | boolean> =>
  promiseScraper(input, 'user', options);
export const trend = async (input: string, options?: Options): Promise<Result | void | boolean> =>
  promiseScraper(input, 'trend', options);
export const music = async (input: string, options?: Options): Promise<Result | void | boolean> =>
  promiseScraper(input, 'music', options);
...
export const getHashtagInfo = async (input: string, options = {} as Options): Promise<HashtagMetadata> => {
  ...
  const contructor: TikTokConstructor = {
    ...getInitOptions(),
    ...options,
    ...{ type: 'signle_hashtag' as ScrapeType, input },
  };
...
export const getMusicInfo = async (input: string, options = {} as Options): Promise<MusicMetadata> => {
  ...
  const contructor: TikTokConstructor = {
    ...getInitOptions(),
    ...options,
    ...{ type: 'single_music' as ScrapeType, input },
  };
  const scraper = new TikTokScraper(contructor);
...
export const getUserProfileInfo = async (input: string, options = {} as Options): Promise<UserMetadata> => {
  ...
  const contructor: TikTokConstructor = {
    ...getInitOptions(),
    ...options,
    ...{ type: 'sinsgle_user' as ScrapeType, input },
  };
  const scraper = new TikTokScraper(contructor);
...
export const signUrl = async (input: string, options = {} as Options): Promise<string> => {
  ...
  const contructor: TikTokConstructor = {
    ...getInitOptions(),
    ...options,
    ...{ type: 'signature' as ScrapeType, input },
  };
  const scraper = new TikTokScraper(contructor);
...
export const getVideoMeta = async (input: string, options = {} as Options): Promise<Result> => {
  ...
  const contructor: TikTokConstructor = {
    ...getInitOptions(),
    ...options,
    ...{ type: 'video_meta' as ScrapeType, input },
  };
  const scraper = new TikTokScraper(contructor);
...
export const video = async (
  input: string,
  options = {} as Options,
): Promise<{ message?: string; json?: string; csv?: string }> => {
  if (options && typeof options !== 'object') {
    throw new TypeError('Object is expected');
  }
...
export const history = async (input: string, options = {} as Options) => {
  ...
  try {
    store = (await fromCallback((cb) =>
      readFile(`${historyPath}/tiktok_history.json`, { encoding: 'utf-8' }, cb),
    )) as string;
  } catch {
  ...
  if (options?.remove) {
    ...
    if (type === 'all') {
      const remove: ReturnType<typeof fromCallback>[] = [];
...
const batchProcessor = (
  batch: Batcher[],
  options = {} as Options,
): Promise<{ type: string; input: string; completed: boolean; scraped?: number }[]> => {
  return new Promise((resolve) => {
    console.log('TikTok Bulk Scraping Started');
    const result: { type: string; input: string; completed: boolean; scraped?: number }[] = [];
    forEachLimit<Batcher>(
      batch,
      options.asyncBulk || 5,
      async (item) => {
        switch (item.type) {
          case 'user':
            try {
              ...
              result.push({
                ...
                input: item.input,
                completed: true,
                scraped: typeof output === 'object' && 'collector' in output ? output.collector.length : undefined,
              });
