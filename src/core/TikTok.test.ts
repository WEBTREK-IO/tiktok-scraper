...
describe('TikTok Scraper MODULE(promise): signUrl', () => {
  ...
  beforeAll(() => {
    instance = new TikTokScraper({
      ...
      filepath: '',
      input:
        'https://m.tiktok.com/share/item/list?secUid=&id=355503&type=3&count=30&minCursor=0&maxCursor=0&shareUid=&lang=',
      type: 'signature',
      headers: {
        'user-agent': 'okhttp',
      },
...
describe('TikTok Scraper MODULE(promise): getHashtagInfo', () => {
  ...
  it('getHashtagInfo should return a valid Object', async () => {
    ...
    expect(hashtag).toEqual({
      ...
      shareMeta: {
        title: '#duett on TikTok',
        desc: '37099.0m views - Watch awesome short videos created with trending hashtag #duett',
      },
