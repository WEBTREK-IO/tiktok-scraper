...
export class TikTokScraper extends EventEmitter {
  ...
  private returnInitError(error: string) {
  ...
  public async scrape(): Promise<Result | void | boolean> {
    if (this.cli && !this.bulk) {
      this.spinner.start();
    }
    ...
    if (this.download && !this.zip) {
      try {
        ...
        return this.returnInitError(err.message);
      }
    }
          ...
          switch (this.scrapeType) {
            case 'user':
              this.getUserId()
                ...
                .then((kill) => cb((kill as unknown as Error) || null))
                .catch((error) => cb(error));
              break;
            case 'hashtag':
              this.getHashTagId()
                .then((query) =>
                  this.submitScrapingRequest({ ...query, cursor: item === 1 ? 0 : (item - 1) * query.count! }, true),
                )
                .then((kill) => cb((kill as unknown as Error) || null))
                .catch((error) => cb(error));
              break;
  ...
  private async submitScrapingRequest(query: RequestQuery, updatedApiResponse = false): Promise<boolean> {
    try {
      ...
      const err = error as Error;
      throw err.message ? new Error(err.message) : error;
    }
  }
    ...
    if (this.collector.length) {
      switch (this.filetype) {
        case 'json':
          ...
          break;
        case 'all':
          await Promise.all([await fromCallback((cb) => writeFile(json, JSON.stringify(this.collector), cb))]);
          break;
        default:
          break;
      }
    ...
    if (this.storeValue && totalNewDownloadedVideos) {
      ...
      try {
        await fromCallback((cb) =>
          writeFile(`${this.historyPath}/${this.storeValue}.json`, JSON.stringify(this.store), cb),
        );
      } catch {
        // continue regardless of error
      }
  ...
  private async scrapeData<T>(qs: RequestQuery): Promise<T> {
    ...
    const unsignedURL = `${this.getApiEndpoint}?${new URLSearchParams(qs as URLSearchParams).toString()}`;
    const signature = sign(unsignedURL, this.headers['user-agent']);
  ...
  public async getMusicInfo(): Promise<MusicMetadata> {
    ...
    const unsignedURL = `${query.uri}?${new URLSearchParams(query.qs as never).toString()}`;
    const signature = sign(unsignedURL, this.headers['user-agent']);

    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-expect-error
    // eslint-disable-next-line no-underscore-dangle
    query.qs._signature = signature;
